...
abstract class BaseUserExt extends ::BaseObject  implements ::Persistent {
	...
	/**
	 * Initializes internal state of BaseUserExt object.
	 * @see        applyDefaults()
	 */
	 ...
	 * @return     UserExt The current object (for fluent API support)
	 */
	public function setKeyname($v)
	{
		...
		if ($this->keyname !== $v) {
			$this->keyname = $v;
			$this->modifiedColumns[] = UserExtPeer::KEYNAME;
		}
	...
	public function setNameId($v)
	{
		...
		if ($this->name_id !== $v) {
			$this->name_id = $v;
			$this->modifiedColumns[] = UserExtPeer::NAME_ID;
		}
	...
	public function setCatId($v)
	{
		...
		if ($this->cat_id !== $v) {
			$this->cat_id = $v;
			$this->modifiedColumns[] = UserExtPeer::CAT_ID;
		}
	...
	public function setHide($v)
	{
		...
		if ($this->hide !== $v) {
			$this->hide = $v;
			$this->modifiedColumns[] = UserExtPeer::HIDE;
		}
	...
	public function setFormat($v)
	{
		...
		if ($this->format !== $v) {
			$this->format = $v;
			$this->modifiedColumns[] = UserExtPeer::FORMAT;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 5; // 5 = UserExtPeer::NUM_COLUMNS - UserExtPeer::NUM_LAZY_LOAD_COLUMNS).
	...
	public function reload($deep = false, ::PropelPDO $con = null)
	{
		...
		$stmt = UserExtPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function save(::PropelPDO $con = null)
	{
		...
		try {
			...
			$con->commit();
			UserExtPeer::addInstanceToPool($this);
			return $affectedRows;
		} catch (::PropelException $e) {
			$con->rollback();
			throw $e;
	...
	protected function doSave(::PropelPDO $con)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					$pk = UserExtPeer::doInsert($this, $con);
					...
					$affectedRows += UserExtPeer::doUpdate($this, $con);
				}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = UserExtPeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = UserExtPeer::translateFieldName($name, $type, ::BasePeer::TYPE_NUM);
	...
	{
		$keys = UserExtPeer::getFieldNames($keyType);
		$result = array(
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(UserExtPeer::KEYNAME)) $criteria->add(UserExtPeer::KEYNAME, $this->keyname);
		if ($this->isColumnModified(UserExtPeer::NAME_ID)) $criteria->add(UserExtPeer::NAME_ID, $this->name_id);
		if ($this->isColumnModified(UserExtPeer::CAT_ID)) $criteria->add(UserExtPeer::CAT_ID, $this->cat_id);
		if ($this->isColumnModified(UserExtPeer::HIDE)) $criteria->add(UserExtPeer::HIDE, $this->hide);
		if ($this->isColumnModified(UserExtPeer::FORMAT)) $criteria->add(UserExtPeer::FORMAT, $this->format);
	...
	public function buildPkeyCriteria()
	{
		...
		$criteria->add(UserExtPeer::KEYNAME, $this->keyname);
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of UserExt (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     UserExt Clone of current object.
	 * @throws     ::PropelException
	 */
	...
	/**
	 * Declares an association between this object and a UserExtCat object.
	 *
	 * @param      UserExtCat $v
	 * @return     UserExt The current object (for fluent API support)
	 * @throws     ::PropelException
	 */
	public function setUserExtCat(UserExtCat $v = null)
	{
		if ($v === null) {
			$this->setCatId(NULL);
		} else {
		...
		// Add binding for other direction of this n:n relationship.
		// If this object has already been added to the UserExtCat object, it will not be re-added.
		if ($v !== null) {
			$v->addUserExt($this);
		}
	...
	/**
	 * Get the associated UserExtCat object
	 *
	 * @param      ::PropelPDO Optional Connection object.
	 * @return     UserExtCat The associated UserExtCat object.
	 * @throws     ::PropelException
	 */
	public function getUserExtCat(::PropelPDO $con = null)
	{
		if ($this->aUserExtCat === null && ($this->cat_id !== null)) {
			$this->aUserExtCat = UserExtCatPeer::retrieveByPK($this->cat_id, $con);
			/* The following can be used additionally to
			   guarantee the related object contains a reference
			   to this object.  This level of coupling may, however, be
			   undesirable since it could result in an only partially populated collection
	 ...
	 * Otherwise if this UserExt has previously been saved, it will retrieve
	 * related UserExtVals from storage. If this UserExt is new, it will return
		...
		if ($this->collUserExtVals === null) {
			if ($this->isNew()) {
			   $this->collUserExtVals = array();
				...
				$criteria->add(UserExtValPeer::KEYNAME, $this->getKeyname());
	 ...
	 */
	public function getUserExtValsJoinUser($criteria = null, $con = null)
	{
		...
		if ($this->collUserExtVals === null) {
			if ($this->isNew()) {
				...
				$this->collUserExtVals = UserExtValPeer::doSelectJoinUser($criteria, $con);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
