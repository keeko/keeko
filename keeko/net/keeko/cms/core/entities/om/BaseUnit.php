...
abstract class BaseUnit extends ::BaseObject  implements ::Persistent {
	...
	/**
	 * Initializes internal state of BaseUnit object.
	 * @see        applyDefaults()
	 */
	...
	public function setGridId($v)
	{
		...
		if ($this->grid_id !== $v) {
			$this->grid_id = $v;
			$this->modifiedColumns[] = UnitPeer::GRID_ID;
		}
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(UnitPeer::GRID_ID)) $criteria->add(UnitPeer::GRID_ID, $this->grid_id);
	 ...
	 * related Menus from storage. If this Unit is new, it will return
		...
		if ($this->collMenus === null) {
			if ($this->isNew()) {
			   $this->collMenus = array();
				...
				$criteria->add(MenuPeer::UNIT_ID, $this->getId());
	 ...
	 */
	public function getMenusJoinBlock($criteria = null, $con = null)
	{
		...
		if ($this->collMenus === null) {
			if ($this->isNew()) {
				...
				$this->collMenus = MenuPeer::doSelectJoinBlock($criteria, $con);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
	 ...
	 * If this collection has already been initialized with an identical ::Criteria, it returns the collection.
	 * Otherwise if this Unit has previously been saved, it will retrieve
	 * related UnitActions from storage. If this Unit is new, it will return
	 * an empty collection or the current collection, the criteria is ignored on a new object.
	 *
	 * @param      ::PropelPDO $con
	 * @param      ::Criteria $criteria
		...
		if ($this->collUnitActions === null) {
			if ($this->isNew()) {
			   $this->collUnitActions = array();
				...
				$criteria->add(UnitActionPeer::UNIT_ID, $this->getId());
	 ...
	 * actually need in Unit.
	 */
	public function getUnitActionsJoinParam($criteria = null, $con = null)
	{
		...
		if ($this->collUnitActions === null) {
			if ($this->isNew()) {
				...
				$this->collUnitActions = UnitActionPeer::doSelectJoinParam($criteria, $con);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
