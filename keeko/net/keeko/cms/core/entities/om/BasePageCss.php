...
abstract class BasePageCss extends ::BaseObject  implements ::Persistent {
	...
	/**
	 * Initializes internal state of BasePageCss object.
	 * @see        applyDefaults()
	 */
	 ...
	 * @return     PageCss The current object (for fluent API support)
	 */
	public function setId($v)
	{
		...
		if ($this->id !== $v) {
			$this->id = $v;
			$this->modifiedColumns[] = PageCssPeer::ID;
		}
	...
	public function setPageId($v)
	{
		...
		if ($this->page_id !== $v) {
			$this->page_id = $v;
			$this->modifiedColumns[] = PageCssPeer::PAGE_ID;
		}
	...
	public function setPath($v)
	{
		...
		if ($this->path !== $v) {
			$this->path = $v;
			$this->modifiedColumns[] = PageCssPeer::PATH;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 3; // 3 = PageCssPeer::NUM_COLUMNS - PageCssPeer::NUM_LAZY_LOAD_COLUMNS).
	...
	public function reload($deep = false, ::PropelPDO $con = null)
	{
		...
		$stmt = PageCssPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function save(::PropelPDO $con = null)
	{
		...
		try {
			...
			$con->commit();
			PageCssPeer::addInstanceToPool($this);
			return $affectedRows;
		} catch (::PropelException $e) {
			$con->rollback();
			throw $e;
	...
	protected function doSave(::PropelPDO $con)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					$pk = PageCssPeer::doInsert($this, $con);
					...
					$affectedRows += PageCssPeer::doUpdate($this, $con);
				}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = PageCssPeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = PageCssPeer::translateFieldName($name, $type, ::BasePeer::TYPE_NUM);
	...
	{
		$keys = PageCssPeer::getFieldNames($keyType);
		$result = array(
			$keys[0] => $this->getId(),
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(PageCssPeer::ID)) $criteria->add(PageCssPeer::ID, $this->id);
		if ($this->isColumnModified(PageCssPeer::PAGE_ID)) $criteria->add(PageCssPeer::PAGE_ID, $this->page_id);
		if ($this->isColumnModified(PageCssPeer::PATH)) $criteria->add(PageCssPeer::PATH, $this->path);
	...
	public function buildPkeyCriteria()
	{
		...
		$criteria->add(PageCssPeer::ID, $this->id);
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of PageCss (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     PageCss Clone of current object.
	 * @throws     ::PropelException
	 */
